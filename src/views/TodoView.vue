...
<template>
  <div class="container">
    <h1>Create Todo List {{ $route.params?.id }}</h1>
    ...
    <form class="form" @submit.prevent="onSubmit" @reset="resetForm">
      <BaseInput v-model="input.title" name="Title" placeholder="Insert Title" required />
      <BaseInput v-model="input.description" name="Description" placeholder="Insert Description" required />
    ...
    <ol class="list">
      ...
      <template v-for="(item, index) in store.getTodo" :key="index">
        ...
        <li :class="{ strike: item?.completed }" @dblclick="toggleComplete(item.id)">
          <button class="red" @click="() => onDelete(item.id, index)" :disabled="editing !== false">&times;</button>
